...
async function retrievePassage(passage, version, timeout) {
  ...
  try {
    ...
    let options = {
      uri: `https://${api_key}@bibles.org/v2/passages.json?q[]=${
        searchPassage[0]
      }+${searchPassage[1]}&version=${ver}`,
      timeout: tOut
    };
      ...
      try {
        ...
        if (language) {
          ...
          versionData.forEach(version => {
            if (
              language.toLowerCase() === version.lang_name.toLowerCase() ||
              language.toLowerCase() === version.lang_name_eng.toLowerCase()
            ) {
